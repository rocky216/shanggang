...
class datafeeds {
  ...
  getBars(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback, firstRequest) {
    const onLoadedCallback = data => {
      data && data.length ? onDataCallback(data) : onDataCallback([], { noData: true })
      // onDataCallback(data);
    }
   ...
   *
   * @param {*Object} symbolInfo 商品信息对象
   * @param {*String} startDate 最左边的请求k线
   * @param {*String} endDate 最右边的请求k线时间
   * @param {*Function} onDataCallback 回调函数
   * @param {*String} resolution 分辨率
   * @memberof datafeeds
   */
  getMarks(symbolInfo, startDate, endDate, onDataCallback, resolution) {
    this.self.getMarks(symbolInfo, startDate, endDate, onDataCallback, resolution);
  }
